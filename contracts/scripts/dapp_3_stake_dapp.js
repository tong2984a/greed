...
async function main() {
  ...
  let player1Address = game_config['dapp_3_stake_dapp']['player1Address']
  let player1Name = game_config['dapp_3_stake_dapp']['player1Name']
  ...
  let value_in_Dai = game_config['dapp_3_stake_dapp']['value_in_Dai']
  ...
  transaction = await daiTokenContract.connect(player1Account).approve(dappFarmAddress, amount)
  tx = await transaction.wait()
  event = tx.events[0]
  console.log("approve event:", event)
  
  transaction = await dappFarmContract.connect(player1Account).stakeTokens(amount)
  tx = await transaction.wait()
  ...
  console.log("\ndapp_3_stake_dapp completed successfully.")
}
